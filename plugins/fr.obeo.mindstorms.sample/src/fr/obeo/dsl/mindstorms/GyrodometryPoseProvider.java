 ...
 * {@link lejos.robotics.navigation.Pose}. It does this using odometry (dead
 * reckoning) data contained in a {@link lejos.robotics.navigation.Move}, which
 * is supplied by a {@link lejos.robotics.navigation.MoveProvider}. When the
 * PoseProivder is constructed, it registers as listener with its MoveProvider,
 */
...
public class GyrodometryPoseProvider implements PoseProvider, MoveListener, SampleProvider {
	...
	GyroDirectionFinder gyrodirect;

	/**
	 * Allocates a new OdometryPoseProivder and registers it with the
	 * MovePovider as a listener.
	 */
	public GyrodometryPoseProvider(MoveProvider mp, GyroDirectionFinder gyrodirect) {
		mp.addMoveListener(this);
		this.gyrodirect = gyrodirect;
		gyrodirect.resetCartesianZero();
	}

	/**
	 * returns a new pose that represents the current location and heading of
	 * the robot. If called while the robot is moving, the PoseProvider will get
	 * updated odometry data from its MoveProvider
	 * 
	 * @return pose
	 */
	 ...
	 * @param mp
	 *            the MoveProvider that called this method
	 */
	...
	private synchronized void updatePose(Move event) {
		float headGyro = gyrodirect.getDegreesCartesian() - angleGyro0;
		...
		angleGyro0 = gyrodirect.getDegreesCartesian();
